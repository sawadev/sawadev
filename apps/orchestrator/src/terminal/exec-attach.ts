...
import { dockerTarget } from '../workspaces/docker';
...
/** Résout le socket unix (Docker Desktop mac le place sous ~/.docker/run). */
...
export async function attachExec(
  ...
  // Connexion unix (socket local) ou TCP (proxy socket).
  const target = dockerTarget();
  const socket: Socket =
    target && 'host' in target
      ? await Bun.connect({ hostname: target.host, port: target.port, socket: sock })
      : await Bun.connect({ unix: resolveDockerSocket(), socket: sock });
