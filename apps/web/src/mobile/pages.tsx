...
import { AgentChat } from '../ide/modules/AgentChat';
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '0 18px 24px' }}>
        ...
        {workspaces.length > 0 && (
          <label className="field" style={{ height: 44, marginBottom: 16 }}>
        ...
        {workspaces.length > 0 && shown.length === 0 && (
          <div style={{ fontSize: 14, color: 'var(--faint)', padding: '8px 0' }}>
...
function MobileIDEBody({ workspaceId }: { workspaceId: string }) {
  ...
  const pane: Record<string, JSX.Element> = {
    ai: <AgentChat workspaceId={workspaceId} />,
    files: (
      <FileTree
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: `4px 18px ${BOTPAD + 16}px` }}>
        ...
        <ApiKeysSection mobile labelStyle={sectionLabel} />
