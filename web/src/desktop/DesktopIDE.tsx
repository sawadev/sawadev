...
function DTerm() {
  const L = ({ children, c, p }: { children: ReactNode; c?: string; p?: boolean }) => (
    <div className="mono" style={{ fontSize: 12, lineHeight: 1.75, color: c || 'rgba(220,220,212,0.9)' }}>
  ...
  return (
    <div style={{ height: 196, flexShrink: 0, borderTop: '1px solid var(--border)', display: 'flex', flexDirection: 'column', background: 'var(--term-bg)' }}>
      <div style={{ height: 34, display: 'flex', alignItems: 'stretch', borderBottom: '1px solid rgba(255,255,255,0.07)', paddingLeft: 4 }}>
        {([['Terminal', true], ['Agent · claude', false], ['Output', false]] as [string, boolean][]).map(([l, on], i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '0 14px', borderBottom: on ? '2px solid var(--accent)' : '2px solid transparent' }}>
            <HIcon name="terminal" size={12} color={on ? 'rgba(235,235,230,0.95)' : 'rgba(220,220,212,0.45)'} />
            <span className="mono" style={{ fontSize: 11.5, color: on ? 'rgba(235,235,230,0.95)' : 'rgba(220,220,212,0.45)', fontWeight: on ? 600 : 400 }}>
              {l}
            </span>
          </div>
        ))}
      ...
      <div style={{ flex: 1, padding: '12px 16px', overflow: 'hidden' }}>
        ...
        <L p c="rgba(240,240,233,0.95)">
          <span style={{ display: 'inline-block', width: 8, height: 15, background: 'rgba(240,240,233,0.9)', verticalAlign: 'text-bottom', animation: 'pulse 1.1s steps(1) infinite' }} />
        </L>
      </div>
    </div>
  );
    ...
    <div style={{ height: '100%', display: 'flex', background: 'var(--bg)' }}>
          ...
          <div style={{ flex: 1, padding: 6, overflow: 'auto' }}>
            {DTREE.map((t, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 7, height: 29, padding: `0 8px 0 ${8 + t.d * 16}px`, borderRadius: 7, background: t.cur ? 'var(--accent-soft)' : 'transparent' }}>
                ...
                <span className="mono" style={{ fontSize: 12.5, fontWeight: t.cur ? 600 : 500, color: t.cur ? 'var(--text)' : 'var(--text-2)', flex: 1 }}>
                  {t.n}
                </span>
                {t.badge && (
                  <span className="mono" style={{ fontSize: 9.5, fontWeight: 700, color: 'var(--accent-text)' }}>
                    {t.badge}
                  </span>
                )}
              </div>
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ height: 40, flexShrink: 0, display: 'flex', alignItems: 'stretch', background: 'var(--surface-2)', borderBottom: '1px solid var(--border)' }}>
            ...
            {['server.ts', 'routes.ts'].map((f, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '0 14px', borderRight: '1px solid var(--border)' }}>
        ...
        <div style={{ width: 392, flexShrink: 0, borderLeft: '1px solid var(--border)', background: 'var(--surface-2)', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 13 }}>
            ...
            <ToolCard m={{ role: 'tool', kind: 'cmd', icon: 'terminal', title: 'Ran command', meta: 'npm i jsonwebtoken', body: 'out' }} />
            <ToolCard m={{ role: 'tool', kind: 'edit', icon: 'diff', title: 'Edited', meta: 'auth/middleware.ts', body: 'diff', approve: true }} />
