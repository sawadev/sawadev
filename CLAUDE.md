...
> Sources de vérité produit/technique : **`SPEC.md`** (décisions) et **`PLAN.md`** (plan d'implémentation, jalons).
...
1. **Lis `SPEC.md` et `PLAN.md` avant de coder.** Ne ré-invente pas une décision déjà prise. Si tu veux dévier, **demande d'abord**.
